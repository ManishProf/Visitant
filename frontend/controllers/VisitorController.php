...
use frontend\models\VisitorSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;
use frontend\models\Checkvisitortime;
use frontend\models\Visittime;
/**
...
class VisitorController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            $model1=$model->Visitor_id;
            $vt=new Visittime();
            $vt->visitorid=$model1;
            $v=Visitor::find()->where(['Visitor_id' =>$model1])->one();
            $vt->visitintime=$v->Date;
            $vt->save();
    ...
    {   
        ...
        if($model->save())
        {
        $v=Visittime::find()->where(['visitorid' =>[$model->Visitor_id]])->one();
        $v->visitouttime=date('Y-m-d H:i:s');
        $v->save();
        } 
